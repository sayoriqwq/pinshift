public enum ControllerTutorial {
  public static let output = """
    ...
    8. Run `pinshift-install` once to build and authorize the controller. Run `pinshift-start` when testing, keep that terminal open, and press Ctrl-C to perform a real Clear before exit.
    ...
    One foreground test session owns Controller Link, in-memory simulation state, and automatic clear. A failed Clear is reported honestly and can be retried from the app; it is not persisted as background work. Historical state never blocks a new Apply. `pinshift-install` preserves the existing Keychain identity, removes old persistent authorities, and stops before mutation if the signing requirement changes. The production Injection Backend is Xcode's public devicectl location workflow. Device selectors, signing details, and pairing material stay private.
    """
}
