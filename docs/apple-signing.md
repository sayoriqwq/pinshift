...
完成首次配置后，可以按使用场景选择续签入口：

| 入口 | 使用条件与行为 |
| --- | --- |
| iPhone「准备使用 Pinshift」快捷指令 | 先完成 [快捷指令配置](iphone-shortcut.md)；Mac 已登录、未睡眠且网络可达。检查签名并按需续签，再启动或复用控制器；App 过期打不开时也可从此入口准备 |
| App「更多 → App 续签 → 立即续签」 | 已配对的 Mac 控制器正在运行且可连接；主动请求续期，安装后重新打开 App 查看结果 |
| Mac `pinshift` / `pinshift app` | 在已配置的项目环境运行；按既有有效期策略检查与续签 |

按需检查在已有 profile 剩余不超过 24 小时时尝试续签。缓存仍有效时可能跳过重建；这不证明手机一定还安装着 App。续签完成后打开 Pinshift，确认能使用并重新连接控制器。Apple 登录、钥匙串或设备授权提示需要在对应设备处理。
