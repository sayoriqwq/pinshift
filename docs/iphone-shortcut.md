...
等待结束仍未确认时，返回非零退出结果 `preparing/manual`。快捷指令保留系统错误提示，不继续打开 App，也不显示成功通知；**准备工作会在 Mac 终端继续，SSH 结束不表示取消或失败结束。** 到 Mac 查看登录、解锁、授权提示，或稍后再次运行快捷指令。准备锁仍被当前请求持有时，重复请求只等待同一次准备结果；上一轮已结束后再次触发会按需重新检查签名并复用控制器。
...
这里交付可照填的动作配置；没有声称提供已在真机签名、导入验证的 `.shortcut` 文件。分享给别人前去除自己的主机、用户名及密钥设置，让对方生成独立密钥并在自己的 Mac 授权。
...
以下是**待执行清单**，不是通过证据。记录 Mac / macOS / Xcode、iPhone / iOS、网络与有线或无线连接方式、提交号、时间，以及每项实际结果和诊断位置；截图和日志不得包含私钥或配对码。

- [ ] 从未启动控制器开始，主屏幕按钮触发准备，Mac 出现可见 Terminal 前台会话。
- [ ] 签名仍有效时跳过重建；到期或进入既有续签阈值时完成签名校验、原位安装确认。
- [ ] 签名/安装超过 30 秒时，手机提示 `preparing/manual` 而非成功；Mac 继续准备，再次触发观察同一请求，完成后能够确认结果。
- [ ] 快捷指令结束和 SSH 断开后，前台会话仍运行；iPhone 实际建立 Controller Link 并选点、Apply、Clear。
- [ ] 连点及活动 Temporary Simulation 中再次触发：无第二个控制器、无并行安装、无额外 Clear，原有截止时间不因准备而改变。
- [ ] App 过期打不开时仍能运行独立快捷指令，续签安装后恢复打开。
- [ ] 原位安装后已保存地点、语言偏好与 Trusted Controller 配对均保留。
- [ ] SSH 断网、错误密钥、未授权用户明确失败；不显示已准备完成。
- [ ] 尝试用专用密钥提交任意命令和请求转发被拒绝，不执行所提交命令。
- [ ] 实测 Apple 登录、手机解锁、Terminal 自动化授权不足时的实际提示与恢复步骤；未遇到的场景标记未测。
- [ ] 签名/安装失败时，反馈保留失败，单独记录控制器是否可用与旧 App 是否可用。
- [ ] 正常 Ctrl-C 退出确认真实 Clear；三分钟 Automatic Clear 行为与既有规则一致。

验收以 iPhone 实际 Controller Link 和应用结果为准；只展示 SSH 输出、模拟外部工具的自动测试、Mac 本地端口可达都不能代替端到端验收。
