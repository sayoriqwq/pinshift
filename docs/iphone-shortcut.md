...
🔧 生成本机专用 `~/.local/bin/pinshift-prepare-remote`，固定仓库、Nix 与开发工具路径和当前设备选择；先设置日常使用所需的 `PINSHIFT_DEVICE`、`PINSHIFT_DEVELOPER_DIR` 再注册。
...
SSH 动作失败时保留系统错误提示，不继续打开 App，也不显示成功通知。准备命令仍可能在 Mac 的可见终端中运行，应查看终端后再决定重试。快捷指令通常在 SSH 动作结束后才提供输出，不能把它当作逐行实时日志：手机最初显示的是「连接中」，返回的已接收/准备中信息描述 Mac 处理阶段。
...
- **`manual` / `failed`（需要人工处理 / 失败）**：查看具体阶段和设备指引；即使已有控制器可用，也不能掩盖 App 维护失败。
...
| 重复请求提示 preparing | 等待原请求结束；若原请求被中断，先查看 Mac 终端与诊断，确认没有准备流程运行后再处理提示的锁文件，勿直接连点或删锁 |
