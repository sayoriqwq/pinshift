...
在 iPhone 的「更多 → App 续签」点击「立即续签」。需要已配对且正在运行的新版 Mac 控制器；旧版控制器会显示更新提示。该操作使用 Mac 本地固定的签名与原位安装流程，不卸载 App，不主动解除模拟，也不远程唤起 Mac。

阶段来自 Mac 回执。离开页面不取消已接收的任务；安装可能关闭 App，请手动重新打开检查结果。「已确认安装」表示 Mac 校验了延长后的签名并确认安装，不代表已打开 App。显示的有效期属于当前 Mac 会话最近确认的安装；没有该证据时显示未知。断线后的旧结果不能证明当前状态，重新连接可查询仍在运行的 Mac 会话。

失败时先查看页面对应的恢复提示；原始错误在「续签详情」和本机诊断记录中。账号确认、证书、设备连接等要求以实际 Mac 输出及设备提示为准。
